    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <?php
            ...
            $uid = $_GET['idz'];
            $zam = mysqli_query($mysql, "SELECT * FROM `zamovlenya` WHERE `id` = '$uid'");
            $zam = mysqli_fetch_assoc($zam);
            ?>      
                    <div class="col">
                    <div class="card mb-4 rounded-3 shadow-sm">
                    <div class="card-header py-3">
                    <h4 class="my-0 fw-normal"> Замовлення номер: <?=$zam['id']?></h4>
                    </div>
                    <div class="card-body">
                    <ul class="list-unstyled mt-3 mb-4">
                      <li>Користувач: <?=$zam['login_user']?> (id: <?=$zam['id_user']?>) </li>
                      <li>П.І.Б. користувача: <?=($_COOKIE['lname_user'])?> <?=($_COOKIE['fname_user'])?> <?=($_COOKIE['oname_user'])?> </li>
                      <li>Тип замовлення: <?=$zam['type']?></li>
                      <li>Статус виконання: <?=$zam['status']?></li>
                      <li>Ціна: <?=$zam['prise']?></li>
                      <br>
                      <li>Коментарі:</li>
                      <p><?=$zam['info_zam']?></p>
                    </ul>
                    <a class="w-100 btn btn-lg btn-outline-primary" href="zamovlenya.php?id=<?=($_COOKIE['id_user'])?>">Вийти</a>
                  </div>
                </div>
              </div>
